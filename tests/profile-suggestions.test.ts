...
import { confirmProfileSuggestion, listConfirmedProfileSuggestions, listProfileEvidence, sanitizeProfileSuggestionDrafts } from "@/lib/profile-suggestions";
...
describe("profile suggestions", () => {
  it("keeps only allowed non-sensitive suggestion kinds and deduplicates values", () => {
    const safe = sanitizeProfileSuggestionDrafts([
      { kind: "current_topic", value: "多模态模型", rationale: "多条确认内容都围绕视觉语言模型" },
      { kind: "current_topic", value: "多模态模型", rationale: "重复建议" },
      { kind: "keyword", value: "RAG 评测", rationale: "确认内容多次出现检索增强评测" },
      { kind: "current_topic", value: "健康状况", rationale: "不允许推断健康信息" },
      { kind: "personality", value: "谨慎", rationale: "不允许的类型" },
      { kind: "keyword", value: "", rationale: "无效" }
    ]);
    expect(safe).toEqual([
      { kind: "current_topic", value: "多模态模型", rationale: "多条确认内容都围绕视觉语言模型" },
      { kind: "keyword", value: "RAG 评测", rationale: "确认内容多次出现检索增强评测" }
